...
if ( ! class_exists( 'Zakra_WooCommerce' ) ) {
	...
	class Zakra_WooCommerce {
		...
		public function __construct() {
			...
			add_filter( 'zakra_woocommerce_header_cart', array( $this, 'woocommerce_header_cart' ) );
			...
			<a class="cart-page-link" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'zakra' ); ?>">
				...
				<span class="count">
					...
					printf(
						...
						'%d',
						WC()->cart->get_cart_contents_count() // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					);
					?>
				</span>
			</a>
		...
		public function woocommerce_header_cart() {
			...
			$output .= $this->woocommerce_cart_link();
			$output .= '</li>';
